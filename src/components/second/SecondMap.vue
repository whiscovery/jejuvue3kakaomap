...
<span v-if="viewmenus == true"><span v-for="menu, i in menus" :key="i" class="badge rounded-pill bg-info mt-1 ms-1 mb-3" @click="filterData(menu)">{{menu}}</span></span>
<div v-else></div>
</div>
 ...
 mounted() {
  ...
  methods: {
    getData() { 
          ...
          this.secondfoods.forEach((element)=>{ // eslint-disable-line no-unused-vars
            var coords={};
            var temp={}
              // 주소로 좌표를 검색합니다
